...
const fullStackWorkflowPath = ".github/workflows/attachment-e2e.yml";
const browserContractWorkflowPath = ".github/workflows/attachment-browser-contract.yml";
const api = fs.readFileSync(apiPath, "utf8");
const editor = fs.readFileSync(editorPath, "utf8");
const renderer = fs.readFileSync(rendererPath, "utf8");
const browserGate = fs.readFileSync(browserGatePath, "utf8");
const editorGate = fs.readFileSync(editorGatePath, "utf8");
const purgeGate = fs.readFileSync(purgeGatePath, "utf8");
const contractServer = fs.readFileSync(contractServerPath, "utf8");
const fullStackWorkflow = fs.readFileSync(fullStackWorkflowPath, "utf8");
const browserContractWorkflow = fs.readFileSync(browserContractWorkflowPath, "utf8");
const combined = `${api}\n${editor}\n${renderer}`;
...
requireText(editorGate, "downloaded.read_text", "downloaded file byte verification");
...
requireText(fullStackWorkflow, "QingZoneX/QTable/.github/actions/export-source@", "scoped private QTable source bootstrap");
requireText(fullStackWorkflow, "QTABLE_BACKEND_SHA", "pinned backend revision evidence");
requireText(fullStackWorkflow, "docker compose up -d --build", "canonical full-stack startup");
requireText(fullStackWorkflow, "Run real AttachmentEditor browser lifecycle", "real editor full-stack browser gate");
...
console.log("[attachment-contract] stable private attachment contract + real editor/full-stack gates OK");
